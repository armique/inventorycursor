/**
 * Verify suggestBuyHelperTargetsFromMarket() — the new market-grounded path added
 * alongside (not replacing) suggestBuyHelperTargets(). Confirms:
 *  - falls back to the existing inventory-only suggestion when no market signal exists
 *  - uses market data (sellEbay/sellKa) when present, computed through the SAME
 *    untouched channelNet/maxBuyForNetMargin/pocketFromXList fee math
 *  - missing one channel's market data falls back per-channel, not all-or-nothing
 * Run: npx tsx scripts/verify-buy-helper-market-bridge.ts
 */
...
console.log('\n=== market = null (never checked), no inventory comps -> falls back to "no comps" message ===');
const noMarket = suggestBuyHelperTargetsFromMarket(null, [], 'RTX 3060 12GB', fees, { desiredMarginPct: desired });
console.log(noMarket);
assert.equal(noMarket.buyPrice, undefined, 'must not invent a buy price with no signal at all');
assert.ok(!noMarket.note?.startsWith('Market:'), 'when market was never checked, note must not claim a market check happened');

console.log('\n=== market checked but BOTH channels empty -> must say so explicitly, not silently ===');
const emptyMarket: ResolvedMarketPrices = { ebay: null, ka: null };
const checkedEmpty = suggestBuyHelperTargetsFromMarket(emptyMarket, [], 'Obscure Part XYZ', fees, { desiredMarginPct: desired });
console.log(checkedEmpty);
assert.equal(checkedEmpty.buyPrice, undefined, 'must not invent a buy price when market was checked and empty, and inventory has no comps either');
assert.ok(
  checkedEmpty.note?.includes('no live/sold data found'),
  `must explicitly say the market was checked and came back empty, got: ${checkedEmpty.note}`,
);
assert.equal(noMarket.inventoryCompCount, 0, 'must surface zero comps, same as suggestBuyHelperTargets would alone');
assert.ok(noMarket.note && noMarket.note.length > 0, 'must still surface some explanatory note, not silence');

console.log('\n=== both channels have market data ===');
const bothChannels: ResolvedMarketPrices = {
  ebay: { source: 'ebay-sold', median: 100, low: 80, high: 120, count: 5 },
  ka: { source: 'ka-live', median: 70, low: 60, high: 80, count: 3 },
};
const both = suggestBuyHelperTargetsFromMarket(bothChannels, [], 'RTX 3060 12GB', fees, { desiredMarginPct: desired });
console.log(both);
...
const ebayOnly: ResolvedMarketPrices = {
  ebay: { source: 'ebay-live', median: 90, low: 70, high: 110, count: 8 },
  ka: null,
};
const onlyEbay = suggestBuyHelperTargetsFromMarket(ebayOnly, [], 'RTX 3060 12GB', fees, { desiredMarginPct: desired });
console.log(onlyEbay);
assert.equal(onlyEbay.sellEbay, 90);
assert.equal(onlyEbay.sellKa, undefined, 'sellKa must stay undefined, not a bogus 0, when neither market nor inventory has KA data');
...
console.log('\nverify-buy-helper-market-bridge: ok');
