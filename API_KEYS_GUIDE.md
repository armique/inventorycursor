...
Powers the "Find real photos" / **Web** button in the item editor — searches real product images and lets you set one as the item's default photo. By default it tries providers in order: **Google Custom Search → Bing Image Search → eBay → Pixabay → Unsplash → Pexels → Openverse → Wikimedia**. Openverse and Wikimedia need **no API key**. eBay uses `EBAY_CLIENT_ID` + `EBAY_CLIENT_SECRET` from `.env` or `dealwatch-runtime/.env` (also works with local `npm run dev`). Optional paid/stock keys still improve match quality.
