/**
 * Product-card spec sanity (server). Keep in sync with utils/productCardSpecSanity.ts.
 */
...
function isDdr2Era(hay) {
  return /\bddr2\b/.test(hay) && !/\bddr[345]\b/.test(hay);
}
function isDdr3Era(hay) {
  return (
...
function looksLikeModernWireless(label, value) {
  const t = `${label} ${value}`;
  return (
...
function looksLikeM2OrNvme(label, value) {
  const t = `${label} ${value}`;
  return /\b(m\.?\s*2|nvme|pcie\s*4|pcie\s*5|pciegen\s*[45]|gen\s*[45]\s*nvme)\b/.test(t);
}
function looksLikePcie4Or5(label, value) {
  const t = `${label} ${value}`;
  return /\b(pcie?\s*(gen\s*)?[45](\.0)?|gen\s*[45]\s*pcie?)\b/.test(t);
}
function looksLikeDdr4Or5(label, value) {
  return /\bddr\s*[45]\b/.test(`${label} ${value}`);
}
function looksLikeDdr3(label, value) {
  return /\bddr\s*3\b/.test(`${label} ${value}`);
}
function looksLikeDdr5(label, value) {
  return /\bddr\s*5\b/.test(`${label} ${value}`);
}
function looksLikeRayTracingOrDlss(label, value) {
  return /\b(ray\s*tracing|rtx|dlss|frame\s*gen|reflex)\b/.test(`${label} ${value}`);
}
function isPreRtxGpu(hay) {
...
function categoryMismatch(ctx, line) {
  ...
  if (isCpu(ctx)) {
    if (/\b(vram|boost clock|cuda|ray tracing|memory type)\b/.test(l) && /\bgddr|gddr\b/.test(v)) return true;
    if (/\b(form factor|chipset|m\.?\s*2)\b/.test(l)) return true;
  }
  if (isRam(ctx) && /\b(socket|chipset|vram|tdp|cuda|form factor)\b/.test(l)) return true;
  if (isStorage(ctx) && /\b(socket|chipset|vram|memory slots?)\b/.test(l)) return true;
  if (isMotherboard(ctx) && /\b(vram|cuda cores|ray tracing|dlss)\b/.test(l)) return true;
  return false;
}

export function sanitizeProductCardSpecs(specs, ctx) {
...
export function buildProductCardSpecGuardrails(ctx, specs) {
  ...
  if (isPreM2EraPlatform(hay)) {
    lines.push(
      '- This product looks pre-M.2 era from its name/specs — forbid M.2, NVMe, PCIe Gen4/5, Wi-Fi 6/7 on the card.'
    );
  }
