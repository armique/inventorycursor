...
function buildDealSearchPrompt(criteria) {
  ...
  const conditionNote = buildConditionPromptNoteFromCriteria(criteria);
...
function buildConditionPromptNoteFromCriteria(criteria) {
  const ids = criteria.itemConditions;
  if (!Array.isArray(ids) || ids.length === 0) return '';
  const labels = {
    new: 'Neu',
    new_other: 'Neu (Sonstige)',
    new_defects: 'Neu mit Mängeln',
    manufacturer_refurbished: 'Vom Hersteller generalüberholt',
    seller_refurbished: 'Vom Verkäufer generalüberholt',
    used: 'Gebraucht',
    very_good: 'Sehr gut',
    good: 'Gut',
    acceptable: 'Akzeptabel',
    for_parts: 'Defekt / Ersatzteile',
  };
  const text = ids.map((id) => labels[id] || id).join(', ');
  return `\nPrefer listings in these conditions (Zustand): ${text}. Skip listings clearly in other conditions.`;
}

function appendEbayConditionsToUrl(url, criteria) {
  const ids = criteria.itemConditions;
  if (!Array.isArray(ids) || ids.length === 0) return url;
  const ebayMap = {
    new: 1000,
    new_other: 1500,
    new_defects: 1750,
  ...
  return ids.reduce((acc, id) => {
    const n = ebayMap[id];
    return n ? `${acc}&LH_ItemCondition=${n}` : acc;
  }, url);
}
...
export function buildMarketplaceSearchLinks(criteria, refinedQuery) {
  ...
  if (platform === 'ebay' || platform === 'both') {
    ...
    url = appendEbayConditionsToUrl(url, criteria);
    deals.push({
