...
export async function handleEbayScreenshot(req, res) {
  ...
  const geminiBody = {
    contents: [{ parts: [{ text: EBAY_ORDER_SCREENSHOT_EXTRACTION_PROMPT }, { inlineData: { mimeType: mime, data: base64 } }] }],
  ...
  try {
    ...
    for (const model of GEMINI_MODELS) {
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        if (!gRes.ok) {
          ...
          if (gRes.status === 429 || gRes.status === 500 || gRes.status === 503) break;
          return res.status(502).json({ error: `Gemini API ${gRes.status}: ${lastErr.slice(0, 200)}` });
        }
        let data;
        try {
    ...
    return res.status(502).json({ error: `Gemini temporarily unavailable. Last: ${lastErr.slice(0, 280)}` });
  } catch (e) {
