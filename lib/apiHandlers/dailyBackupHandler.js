...
export async function handleDailyBackup(req, res) {
  ...
  if (missing.length) {
    // Log as well as return. A cron failure is only ever seen in the Vercel log,
    // and a 500 whose reason exists solely in an unread response body is
    // effectively a silent failure.
    console.error(`[daily-backup] Not configured — missing env: ${missing.join(', ')}`);
  ...
  try {
    ...
    console.error('[daily-backup] Failed:', e instanceof Error ? e.stack || e.message : e);
