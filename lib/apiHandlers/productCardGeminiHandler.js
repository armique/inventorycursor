...
} from '../productCardSpecSanity.js';
import { lookupOfficialProductCardSpecs } from '../productCardSpecLookup.js';
...
export function buildCardPrompt({
  ...
  const safeSpecs = sanitizeProductCardSpecs(Array.isArray(specs) ? specs : [], ctx);
  const specLines = safeSpecs
    .slice(0, 8)
    .map((s) => `• ${s.label}: ${s.value}`)
    .join('\n');
  const guardrails = buildProductCardSpecGuardrails(ctx, safeSpecs);
  const accessoryLines = Array.isArray(accessoryHints)
    ? accessoryHints.filter(Boolean).map((l) => `- ${l}`).join('\n')
    : '';

  const verifyBlock =
    specVerification?.verified
      ? `OFFICIAL SPEC CHECK (already done via Google Search before this image was requested):
- Confidence: ${specVerification.confidence || 'medium'}
- Sources: ${(specVerification.sourceUrls || []).slice(0, 4).join(' · ') || 'manufacturer / datasheet search'}
${specVerification.notes ? `- Notes: ${String(specVerification.notes).slice(0, 240)}` : ''}
- Treat SPECIFICATIONS below as web-checked. Do NOT add features the sources did not confirm.
- Do NOT invent M.2, NVMe, Wi-Fi 6/7, PCIe 4/5, DDR4/5, RTX/DLSS, or other upgrades not listed.`
      : `OFFICIAL SPEC CHECK:
- A web lookup was attempted or unavailable. Still do NOT invent specs beyond SPECIFICATIONS / the product name era.
- Prefer omitting a feature over guessing.`;
...
- Do NOT redesign hardware. Do NOT add missing parts, RAM sticks, coolers, IO shields, or cables unless ACCESSORY FLAGS say they are included and should appear as packaging/badge text (not fake hardware).
...
- Do NOT redesign hardware. Do NOT add missing parts, RAM sticks, coolers, IO shields, or cables as physical parts.
...
SPECIFICATIONS (source of truth — print ONLY these rows; never invent, replace, upgrade, or modernize values):
${specLines || '• Use only details clearly present in the product name — do not invent ports, M.2, Wi-Fi, VRAM, or other features'}
${comment ? `NOTES: ${String(comment).slice(0, 200)}` : ''}
${accessoryLines ? `ACCESSORY FLAGS (buyer-facing — may appear as small packaging badges / Lieferumfang chips on the card; do not invent opposite):\n${accessoryLines}` : ''}
...
- Double-check every on-card spec against SPECIFICATIONS + product era before drawing text.
...
export async function handleProductCardGemini(req, res) {
  ...
  const specs = sanitizeProductCardSpecs(Array.isArray(body.specs) ? body.specs : [], {
    name,
  ...
  const style = getProductCardStyle(styleId);
  const skipLookup = body.skipSpecLookup === true;

  try {
    let finalSpecs = specs;
    let specVerification = null;

    // Image models cannot Google — verify specs first with Gemini + Google Search when possible.
    if (!skipLookup && geminiKey) {
      try {
        const lookup = await lookupOfficialProductCardSpecs(geminiKey, {
          name,
          category,
          subCategory,
          specs,
        ...
        console.warn(
          '[product-card] spec web lookup skipped:',
    ...
    const prompt = buildCardPrompt({
      ...
      editFromPhoto,
      specVerification,
      accessoryHints: Array.isArray(body.accessoryHints) ? body.accessoryHints : undefined,
    });
    ...
    const verifyNote = specVerification?.verified
      ? ` · specs web-checked (${specVerification.confidence})`
      : geminiKey && !skipLookup
        ? ' · spec web-check attempted'
        : '';
    ...
    return res.status(200).json({
      ...
      specsUsed: finalSpecs,
      specVerification,
      note: photoBuffers.length
        ? `${style.name} · specs + your photos${verifyNote}`
        : `${style.name} · name & specs (no photos)${verifyNote}`,
    });
  } catch (e) {
