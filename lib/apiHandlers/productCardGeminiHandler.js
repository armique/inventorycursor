...
export async function handleProductCardGemini(req, res) {
  ...
  // AI Studio keys are commonly AIza…; newer keys may be AQ.… — reject only obvious OAuth/JWT pastes.
  const looksLikeOAuth =
    /^(ya29\.|1\/\/|eyJ)/i.test(apiKey) || apiKey.includes(' ') || apiKey.length < 20;
  if (looksLikeOAuth) {
    return res.status(401).json({
      error:
        'Gemini API key looks like an OAuth/login token, not an AI Studio API key. Create a key at https://aistudio.google.com/apikey',
      hint: 'Put the API key in GEMINI_API_KEY / VITE_GEMINI_API_KEY, then restart the server / redeploy.',
    });
  }
  ...
  try {
    ...
    return res.status(authFailed ? 401 : rateLimited ? 429 : 503).json({
      ...
      hint: authFailed
        ? 'Invalid/expired API key — not a model bug. Gemini app Pro ≠ API key. Use a key from https://aistudio.google.com/apikey (AIza… or AQ.…).'
