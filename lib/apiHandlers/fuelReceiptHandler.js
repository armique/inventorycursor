...
export async function handleFuelReceipt(req, res) {
  ...
  try {
    const { parsed: o } = await callGeminiVisionJson({
      apiKey,
      prompt: FUEL_RECEIPT_EXTRACTION_PROMPT,
      mime,
      base64,
    });
    ...
    return res.status(200).json({
      parsed: {
        receiptDate: parseExtractedFuelReceiptDate(row.receiptDate),
        fuelAmountEur: parseExtractedFuelEurAmount(row.fuelAmountEur),
        stationName: str(row.stationName),
        fuelLabel: str(row.fuelLabel),
      },
    });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Gemini failed';
    console.error('fuel-receipt', e);
