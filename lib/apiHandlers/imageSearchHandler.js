...
let mergedPhotoEnv = false;
function mergePhotoSearchEnvOnce() {
  if (mergedPhotoEnv) return;
  mergedPhotoEnv = true;
  const cwd = process.cwd();
  for (const rel of ['.env.local', '.env', join('dealwatch-runtime', '.env')]) {
...
function ebayConfig() {
  return {
    clientId: pick('EBAY_CLIENT_ID', 'VITE_EBAY_CLIENT_ID'),
    clientSecret: pick('EBAY_CLIENT_SECRET', 'VITE_EBAY_CLIENT_SECRET'),
    marketplace: pick('EBAY_MARKETPLACE_ID', 'VITE_EBAY_MARKETPLACE_ID') || 'EBAY_DE',
  };
}

/** Openverse (Creative Commons) — official API, no key. */
async function searchOpenverse(q, num) {
  const url = new URL('https://api.openverse.org/v1/images/');
  url.searchParams.set('q', q);
  url.searchParams.set('page_size', String(num));
  const r = await fetch(url.toString(), {
    headers: { 'User-Agent': 'DeInventory-Pro/1.0 (local inventory photo lookup)' },
  });
  const data = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(data?.detail || `Openverse API ${r.status}`);
  const items = Array.isArray(data.results) ? data.results : [];
  return items
    .map((it) => ({
      url: it.url,
      ...
      title: it.title || '',
      contextLink: it.foreign_landing_url || '',
    }))
    .filter((row) => typeof row.url === 'string' && row.url.trim());
}

/** Wikimedia Commons — official API, no key. Last-resort product/stock photos. */
async function searchWikimedia(q, num) {
  ...
  url.searchParams.set('gsrlimit', String(num));
  ...
  return pages
    .map((it) => {
      const info = Array.isArray(it.imageinfo) ? it.imageinfo[0] : null;
      const mime = String(info?.mime || '');
      if (mime && !mime.startsWith('image/')) return null;
      if (mime.includes('svg')) return null;
      const urlOut = info?.thumburl || info?.url;
      if (typeof urlOut !== 'string' || !urlOut.trim()) return null;
      return {
        url: urlOut,
        thumbnail: info?.thumburl || urlOut,
        title: String(it.title || '').replace(/^File:/, ''),
...
const PROVIDERS = [
  ...
  { name: 'openverse', label: 'Openverse', run: searchOpenverse, isConfigured: () => true },
  { name: 'wikimedia', label: 'Wikimedia', run: searchWikimedia, isConfigured: () => true },
];
...
export async function handleImageSearch(req, res) {
  ...
  if (!anyConfigured) {
    return res.status(500).json({
      error:
        'No photo search provider configured. Add EBAY_CLIENT_ID+EBAY_CLIENT_SECRET, or GOOGLE_SEARCH_API_KEY+GOOGLE_SEARCH_CX (see API_KEYS_GUIDE.md).',
    });
  }
