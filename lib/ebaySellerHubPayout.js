...
export function extractHubOrderLifecycle(text) {
  ...
  const refundEur = absMoney(
    amountAfter(src, /Erstattet(?:er)?\s+Betrag|Rückerstattungsbetrag|Refund(?:ed)?\s+amount|Amount refunded/i) ??
      amountAfter(src, /(?:^|\n)\s*Rückerstattung\b/i) ??
      amountAfter(src, /(?:^|\n)\s*Erstattet\b/i)
  );

  let status = 'unknown';
  if (/teilweise\s+erstattet|partial(?:ly)?\s+refund|(?:^|\n)\s*Rückerstattung\b/i.test(src)) {
  ...
  else if (/\berstattet\b|\brefunded\b/i.test(src)) status = 'refunded_full';
...
/**
 * Bestelleinnahmen is authoritative. When it equals Gesamtbetrag, fee lines are spurious.
 * When explicit net disagrees with parsed fee buckets, align fees to total − net.
 * @param {ReturnType<typeof parseEbaySellerHubPayoutText>} payout
 */
export function reconcileHubPayoutFees(payout) {
  if (!payout) return payout;
  const total = payout.buyerTotalEur ?? payout.itemGrossEur;
  let net = payout.netPayoutEur;
  if (total == null) return payout;

  if (net != null && Math.abs(total - net) < 0.02) {
    return {
      ...payout,
      netPayoutEur: roundMoney(net),
  ...
  if (net != null && net < total - 0.005) {
    const implied = roundMoney(total - net);
    const parsed = roundMoney(
      (payout.transactionFeeEur ?? 0) +
        (payout.adFeeEur ?? 0) +
        (payout.shippingLabelEur ?? 0) +
        (payout.otherFeeEur ?? 0)
    );
    if (Math.abs(parsed - implied) >= 0.05) {
      return {
        ...payout,
        netPayoutEur: roundMoney(net),
        transactionFeeEur: implied >= 0.01 ? implied : null,
        adFeeEur: null,
        shippingLabelEur: null,
        otherFeeEur: null,
      };
  ...
  if (net == null) {
    ...
    if (parsed < 0.01) {
      return { ...payout, netPayoutEur: roundMoney(total) };
    }
  }
...
export function applyBusinessTxFeePolicy(payout, creationDate) {
  if (!payout) return payout;
  let next = payout;
  if (creationDate && creationDate < EBAY_DE_BUSINESS_TX_FEE_FROM) {
    if (next.transactionFeeEur != null && next.transactionFeeEur >= 0.01) {
      next = { ...next, transactionFeeEur: null };
    }
    return next;
  }
  return reconcileHubPayoutFees(next);
}
...
export function parseEbaySellerHubPayoutText(text) {
  ...
  const refundEur = absMoney(
    amountAfter(buyerSection, /Rückerstattung|Partial\s*refund|(?<![A-Za-z])Refund(?!\s*amount)/i) ??
      amountAfter(src, /(?:^|\n)\s*Rückerstattung\b/i)
  );
...
Rules:
- buyerTotalEur = Gesamtbetrag under "Vom Käufer bezahlt" (item + shipping − refund). The Gesamtbetrag under "Ihr Verkaufserlös" is the same number — do not add it twice.
- itemGrossEur = Zwischensumme / Artikelpreis if shown.
- buyerShippingEur = Versand the buyer paid (not Versandetikett).
- refundEur = Rückerstattung in the buyer-paid section, positive number. Gesamtbetrag is already after this refund.
...
- netPayoutEur = Bestelleinnahmen. That is the wallet net after fees. Do not subtract Rückerstattung again.
...
export function payoutFromHubVisionJson(raw, orderId) {
  ...
  const refundEur = numField(o.refundEur);
