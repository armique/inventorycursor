...
const EBAY_ORDER_SYNC_FROM = '2026-08-28';
const DEFAULT_LOOKBACK_DAYS = 30;
const ISO_DAY = /^\d{4}-\d{2}-\d{2}$/;

function normalizeEbayOrderDate(raw) {
...
function normalizeOrderRecord(o) {
  return {
    ...o,
    creationDate: normalizeEbayOrderDate(o.creationDate),
    lastModifiedDate: normalizeEbayOrderDate(o.lastModifiedDate),
    financialEvents: o.financialEvents?.map((e) => ({
      ...e,
      date: normalizeEbayOrderDate(e.date),
    })),
  };
}

function clampEbaySyncFromDate(fromDate) {
  const raw = String(fromDate || EBAY_ORDER_SYNC_FROM).trim().slice(0, 10);
  return raw < EBAY_ORDER_SYNC_FROM ? EBAY_ORDER_SYNC_FROM : raw;
}

function incrementalSyncFromDate(existingOrders, fallbackFrom) {
  let newest = null;
  for (const o of existingOrders) {
    const d = o.creationDate?.slice?.(0, 10);
    if (d && (!newest || d > newest)) newest = d;
  }
  if (!newest) return clampEbaySyncFromDate(fallbackFrom);
  const resume = new Date(`${newest}T00:00:00Z`);
  resume.setUTCDate(resume.getUTCDate() - 1);
  const from = resume.toISOString().split('T')[0];
  return clampEbaySyncFromDate(from);
}

function isEbayOrderOnOrAfterSyncFrom(creationDate) {
  if (!creationDate) return false;
  return String(creationDate).trim().slice(0, 10) >= EBAY_ORDER_SYNC_FROM;
}
  ...
  for (;;) {
    ...
    for (const order of orders) {
      ...
      allOrders.push({
        ...
        creationDate: normalizeEbayOrderDate(order.creationDate),
        lastModifiedDate: normalizeEbayOrderDate(order.lastModifiedDate),
...
export function financeTransactionsToOrderRecords(transactions, importedAt = new Date().toISOString()) {
  ...
  for (const tx of transactions) {
    ...
    const date = normalizeEbayOrderDate(tx.transactionDate);
...
export function upsertOrdersInMap(byId, newOrders, options = {}) {
  const insertOnly = options.insertOnly === true;
  let added = 0;
  let merged = 0;
  const changed = [];
  for (const inc of newOrders) {
    const normalized = normalizeOrderRecord(inc);
    const existing = byId.get(normalized.orderId);
    if (existing) {
      if (insertOnly) continue;
      const mergedRecord = mergeOrderRecords(existing, normalized);
      byId.set(normalized.orderId, mergedRecord);
...
function baseTxRow(order, dayRaw) {
  const lineItems = order.lineItems || [];
  const { createdAt, createdSort } = ebayTxRowDates(dayRaw);
  return {
...
function orderRecordToTxRows(order) {
  ...
  } else if (order.feeTotal != null) {
    fvfEur = -Math.abs(order.feeTotal);
  }

  const lineItemSubtotal =
    lineItems.reduce((sum, li) => sum + (Number(li.lineItemCost) || 0), 0) || null;
  const impliedShipping =
    lineItemSubtotal != null && saleGross - lineItemSubtotal > 0.01
      ...
      : lineItemSubtotal != null
        ? 0
        : null;
  const shipping = order.shippingCost ?? impliedShipping;

  if (!lineItems.length) {
    const row = baseTxRow(order, day);
    row.itemSubtotalEur = order.grossTotal ?? null;
    row.shippingEur = shipping;
    row.netEur = order.netTotal ?? (hasFinancesData ? roundMoney(saleGross + fvfEur) : null);
    row.variableFeeEur = fvfEur || null;
    row.grossEur = order.grossTotal ?? (saleGross || null);
    return [row, ...extraRows];
  }

  const orderRows = lineItems.map((li, i) => {
    const row = baseTxRow(order, day);
    const lineKey = li.listingId || li.sku || String(i);
    row.id = `api-${order.orderId}-${lineKey}`;
    row.title = li.title || '';
    row.listingId = li.listingId || '';
    row.sku = li.sku || '';
    row.quantity = li.quantity ?? 1;
    row.itemSubtotalEur = li.lineItemCost ?? null;
    row.shippingEur = i === 0 ? shipping : null;
    row.variableFeeEur = i === 0 ? fvfEur || null : null;
    const lineGross = roundMoney((Number(li.lineItemCost) || 0) + (i === 0 ? Number(shipping) || 0 : 0));
    row.grossEur = lineGross || null;
    row.netEur = roundMoney(lineGross + (i === 0 ? fvfEur : 0));
    return row;
  });
...
export function rebuildApiSyncReport(allOrders, existingReports) {
  ...
  const newlyDiscovered = apiOrders.filter((o) => o.orderId && !previousApiOrderIds.has(o.orderId));
  const newRows = newlyDiscovered.flatMap(orderRecordToTxRows);
  const rows = [...(previousApiReport?.rows || []), ...newRows];
  ...
  const newOrderIds = newlyDiscovered.map((o) => o.orderId).filter(Boolean);
...
export async function runEbayOrderCloudSync(sb, userId, options = {}) {
  ...
  const fallbackFrom = options.fromDate ?? isoDateDaysAgo(lookbackDays);
  const fromDate = incrementalSyncFromDate(existingOrders, fallbackFrom);
  const toDate = options.toDate ?? isoDateDaysAgo(0);
  const importedAt = new Date().toISOString();

  const fulfillmentSummaries = (await fetchEbayFulfillmentOrders(accessToken, fromDate, toDate)).filter((o) =>
    isEbayOrderOnOrAfterSyncFrom(o.creationDate ? o.creationDate.split('T')[0] : null)
  );
  const fulfillmentRecords = fulfillmentSummaries.map((o) => fulfillmentSummaryToRecord(o, importedAt));
  const fulfillmentResult = upsertOrdersInMap(byId, fulfillmentRecords, { insertOnly: true });

  let feesWarning;
  if (fulfillmentResult.added > 0) {
    try {
      ...
      upsertOrdersInMap(byId, financeRecords, { insertOnly: true });
    } catch (e) {
      feesWarning =
