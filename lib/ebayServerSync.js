...
export async function fetchEbayFulfillmentOrders(token, fromDate, toDate) {
  const now = new Date();
  const to = toDate
    ? new Date(Math.min(new Date(`${toDate}T23:59:59.999Z`).getTime(), now.getTime()))
    : now;
  const from = fromDate ? new Date(`${fromDate}T00:00:00.000Z`) : new Date(now.getTime() - 7 * 86400000);
